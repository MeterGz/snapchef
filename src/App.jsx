          ...
          return (
            <div className="fade-up" style={{...fcol,gap:28}}>
              ...
              <div className="flex gap-3" style={{display:'flex',gap:12}}>
                {onboardingStep > 0 && (
                  <button onClick={() => setOnboardingStep(onboardingStep - 1)} className={`flex-1 ${bgSubtle2Class} ${textClass} rounded-2xl py-4 font-semibold`} style={{flex:1,padding:'16px 0',borderRadius:16,fontWeight:600,fontSize:15,border:'none',cursor:'pointer',background: darkMode ? '#222838' : '#F3F4F6',color: textCol}}>Back</button>
                )}
                <button onClick={() => {
                  if (onboardingStep < steps.length - 1) setOnboardingStep(onboardingStep + 1);
                  else completeOnboarding(userProfile);
                }} className="flex-1 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-2xl py-4 font-bold shadow-lg shadow-red-500/20" style={{flex:1,padding:'16px 0',borderRadius:16,fontWeight:700,fontSize:15,border:'none',cursor:'pointer',background: gradientBtn,color: gradientBtnCol,boxShadow: gradientShadow}}>
                  {onboardingStep < steps.length - 1 ? 'Continue' : "Let's Cook! 🎉"}
                </button>
              </div>
              {onboardingStep === 0 && (
                <button onClick={() => { setOnboardingDone(true); saveToStorage('onboarding-done', true); }} className={`w-full text-sm ${textMutedClass}`} style={{padding:'8px 0',width:'100%',background:'none',border:'none',cursor:'pointer',fontSize:14,...t.textMuted}}>Skip for now</button>
              )}
            </div>
          );
        })()}
