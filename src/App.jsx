          ...
          return (
            <div className="fade-up" style={{...fcol,gap:28}}>
              ...
              <div className="flex gap-3" style={{display:'flex',gap:12}}>
                {onboardingStep > 0 && (
                  <button onClick={() => setOnboardingStep(onboardingStep - 1)} className={`flex-1 ${bgSubtle2Class} ${textClass} rounded-2xl py-4 font-semibold`} style={{flex:1,background: subtleBg2,borderRadius:16,padding:'16px 0',fontWeight:600,fontSize:15,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',...t.text}}>Back</button>
                )}
                <button onClick={() => {
                  if (onboardingStep < steps.length - 1) setOnboardingStep(onboardingStep + 1);
                  else completeOnboarding(userProfile);
                }} className="flex-1 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-2xl py-4 font-bold shadow-lg shadow-red-500/20" style={{flex:1,background: gradientBtn,color:'#fff',borderRadius:16,padding:'16px 0',fontWeight:700,fontSize:15,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',boxShadow: gradientShadow}}>
                  {onboardingStep < steps.length - 1 ? 'Continue' : "Let's Cook! 🎉"}
                </button>
              </div>
              {onboardingStep === 0 && (
                <button onClick={() => { setOnboardingDone(true); saveToStorage('onboarding-done', true); }} className={`w-full text-sm ${textMutedClass}`} style={{padding:'8px 0',width:'100%',background:'none',border:'none',cursor:'pointer',fontSize:14,fontFamily:'Manrope, sans-serif',...t.textMuted}}>Skip for now</button>
              )}
            </div>
          );
        })()}
          ...
          <div className="space-y-6 fade-up screen-enter">
            ...
            {images.length < MAX_PHOTOS && (
              <div className="grid gap-3">
                <button onClick={() => cameraInputRef.current?.click()} className="card-hover flex items-center justify-center gap-3 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-2xl p-5 shadow-lg shadow-red-500/20" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:12,background: gradientBtn,color:'#fff',borderRadius:16,padding:20,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',boxShadow: gradientShadow}}>
                  <Camera className="w-6 h-6" /><span style={{fontSize:18,fontWeight:700}}>Take Photo</span>
                </button>
                <button onClick={() => fileInputRef.current?.click()} className={`card-hover flex items-center justify-center gap-3 ${cardClass} border-2 rounded-2xl p-5 shadow-sm`} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:12,background: subtleBg,borderRadius:16,padding:20,border:`2px solid ${borderLight}`,cursor:'pointer',fontFamily:'Manrope, sans-serif'}}>
            ...
            {images.length > 0 && (
              ...
              <button
                ...
                style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',background:'linear-gradient(to right, #10b981, #14b8a6)',color:'#fff',borderRadius:16,padding:20,fontWeight:700,fontSize:18,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',boxShadow:'0 8px 30px rgba(16,185,129,0.2)'}}
              >
