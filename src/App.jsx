...
export default function SnapChef() {
  ...
  const fetchProfile = async (userId) => {
    if (!supabase) return;
    try {
      const { data } = await supabase.from('profiles').select('is_pro, display_name, daily_scans_used, last_scan_date, pro_expires_at').eq('id', userId).single();
      if (data) {
        // Check if Pro has expired (webhook may have failed/delayed)
        let proStatus = data.is_pro || false;
        if (proStatus && data.pro_expires_at) {
          const expiresAt = new Date(data.pro_expires_at * 1000 || data.pro_expires_at);
          if (expiresAt < new Date()) {
            proStatus = false;
            // Update DB to reflect expiration
            supabase.from('profiles').update({ is_pro: false, stripe_subscription_id: null, pro_expires_at: null, updated_at: new Date().toISOString() }).eq('id', userId).then(() => {});
          }
        }
        setIsPro(proStatus);
        ...
        // Sync scan count from server (prevents localStorage bypass)
        const today = new Date().toISOString().split('T')[0];
        if (data.last_scan_date === today && data.daily_scans_used > 0) {
          const serverScans = { date: today, count: data.daily_scans_used };
          setDailyScans(prev => {
            const localCount = prev.date === today ? prev.count : 0;
            return serverScans.count >= localCount ? serverScans : prev;
          });
        }
      }
  ...
  const handleAuth = async (e) => {
    ...
    try {
      let result;
      if (authMode === 'forgot') {
        if (!authEmail) { setAuthError('Enter your email address'); setAuthSubmitting(false); return; }
        const { error } = await supabase.auth.resetPasswordForEmail(authEmail, {
          redirectTo: window.location.origin + '?reset=true'
        });
        if (error) throw error;
        setAuthError('Password reset link sent! Check your email.');
        track('password_reset_requested', { email: authEmail });
      } else if (authMode === 'signup') {
        if (authPassword.length < 6) { setAuthError('Password must be at least 6 characters'); setAuthSubmitting(false); return; }
        if (authPassword.length > 72) { setAuthError('Password must be 72 characters or less'); setAuthSubmitting(false); return; }
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword.length < 6) { showToast('Password must be at least 6 characters', 'info'); return; }
    if (newPassword !== confirmPassword) { showToast('Passwords don\'t match', 'info'); return; }
    try {
  ...
  const handleUpdateProfile = async () => {
    ...
    if (!trimmed) { showToast('Name cannot be empty', 'info'); return; }
    try {
      await supabase.from('profiles').update({ display_name: trimmed, updated_at: new Date().toISOString() }).eq('id', authUser.id);
      setUserProfile(prev => ({ ...prev, name: trimmed }));
      saveToStorage('user-profile', { ...userProfile, name: trimmed });
  ...
  const handleDeleteAccount = async () => {
    if (!supabase || !authUser) return;
    if (deleteConfirmText !== 'DELETE') { showToast('Type DELETE to confirm', 'info'); return; }
    try {
      ...
      showToast('Failed to delete account. Contact support.', 'info');
    }
  };
  ...
  useEffect(() => {
    ...
    if (params.get('upgraded') === 'true') {
      // Poll Supabase to confirm Pro status (webhook may take a few seconds)
      const checkProStatus = async (attempts = 0) => {
        if (!supabase || attempts > 8) return; // Give up after ~16s
        try {
          const { data: { session } } = await supabase.auth.getSession();
          if (session?.user) {
            ...
            if (profile?.is_pro) {
              setIsPro(true);
              showToast('Welcome to Pro! Enjoy unlimited scans.');
      ...
      setTimeout(() => checkProStatus(), 1500);
  ...
  const handlePantryScan = async (e) => {
    ...
    try {
      ...
      const scanHeaders = await getScanHeaders();
...
${langHint} Return ONLY a JSON array: ["ingredient1", "ingredient2", ...]`;
  ...
  const analyzeAllImages = async () => {
    ...
    try {
      ...
      let allIngredients;
      const mainScanHeaders = await getScanHeaders();
      if (scanImages.length === 1) {
        // Single image — one call
        const response = await fetchWithRetry("/api/anthropic", {
          method: "POST",
          headers: mainScanHeaders,
          body: JSON.stringify({
            model: scanModel,
            max_tokens: 800,
            messages: [{
...
Return ONLY the JSON array, no other text.` }]
          ...
          const claudeHeaders = await getApiHeaders();
  ...
  const getFullRecipe = async (recipe) => {
    ...
    try {
      if (recipe.spoonacularId) {
        ...
        const recipeHeaders = await getApiHeaders();
        const response = await fetchWithRetry("/api/anthropic", {
          method: "POST",
  ...
  const autoFillMealPlan = async () => {
    ...
    try {
      ...
      const mealPlanHeaders = await getApiHeaders();
      const response = await fetchWithRetry("/api/anthropic", {
        method: "POST",
...
[{"day":1,"meal":"${mealSlots[0]}","name":"Recipe Name","cookTime":"15 mins","servings":1,"difficulty":"Easy","cuisine":"American","description":"Brief desc","calories":450,"protein":30,"carbs":40,"fat":15}]`
  ...
  const scanMealPhoto = async () => {
    ...
    try {
      const mealScanHeaders = await getScanHeaders();
      const response = await fetchWithRetry("/api/anthropic", {
        method: "POST",
        headers: mealScanHeaders,
        body: JSON.stringify({
  ...
  const getDrinkPairing = async (recipe) => {
    ...
    try {
      const pairingHeaders = await getApiHeaders();
      const response = await fetchWithRetry("/api/anthropic", {
        method: "POST",
  ...
  const sendRemix = async () => {
    ...
    try {
      ...
      const remixHeaders = await getApiHeaders();
      const response = await fetchWithRetry("/api/anthropic", {
        method: "POST", headers: remixHeaders,
        body: JSON.stringify({
  ...
  const getScanHeaders = async () => {
    const headers = await getApiHeaders();
    headers['X-Scan-Request'] = 'true';
    return headers;
  };
  const useScan = () => {
    const today = new Date().toISOString().split('T')[0];
    ...
    if (supabase && authUser) {
      supabase.from('profiles').update({ daily_scans_used: updated.count, last_scan_date: today }).eq('id', authUser.id).then(() => {});
    }
    return true;
  };
      ...
      <div className="max-w-2xl mx-auto px-5 py-7" style={{position:'relative',zIndex:1,paddingBottom:120}}>
          ...
          <div style={{position:'fixed',inset:0,zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(4px)'}}
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              <div style={{textAlign:'center',marginBottom:24}}>
                ...
                <h3 style={{fontSize:22,fontWeight:800,marginTop:12,...t.text}}>{authMode === 'forgot' ? 'Reset password' : authMode === 'login' ? 'Welcome back' : 'Create account'}</h3>
                <p style={{fontSize:13,marginTop:6,...t.textMuted}}>{authMode === 'forgot' ? "We'll send you a reset link" : authMode === 'login' ? 'Sign in to sync your data' : 'Sign up to unlock Pro features'}</p>
              </div>
              {authError && (
                <div style={{padding:'10px 14px',borderRadius:10,marginBottom:16,fontSize:13,fontWeight:600,
                  background: (authError.includes('Check your email') || authError.includes('reset link sent')) ? (darkMode ? 'rgba(52,211,153,0.1)' : 'rgba(22,163,74,0.05)') : (darkMode ? 'rgba(255,92,114,0.1)' : 'rgba(239,68,68,0.05)'),
                  color: (authError.includes('Check your email') || authError.includes('reset link sent')) ? successCol : '#EF4444'}}>
                  {authError}
                </div>
              )}
                ...
                {authMode !== 'forgot' && (
                  <>
                    <button onClick={handleGoogleSignIn}
                      style={{width:'100%',padding:'14px 16px',borderRadius:12,fontSize:14,fontWeight:600,border:`1.5px solid ${borderCol}`,
                        background: subtleBg,color: textCol,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:10}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:12,margin:'4px 0'}}>
                      ...
                      <span style={{fontSize:12,...t.textMuted}}>or</span>
                      <div style={{flex:1,height:1,background: borderCol}} />
                    </div>
                  </>
                )}
                ...
                {authMode !== 'forgot' && (
                  <>
                    <input type="password" placeholder="Password" value={authPassword} onChange={e => setAuthPassword(e.target.value)}
                      ...
                      style={{width:'100%',padding:'14px 16px',borderRadius:12,fontSize:15,border:`1.5px solid ${borderCol}`,background: subtleBg,color: textCol,outline:'none',boxSizing:'border-box'}} />
                    {authMode === 'signup' && (
                      <p style={{fontSize:11,...t.textMuted,margin:'-4px 0 0 4px'}}>Minimum 6 characters</p>
                    )}
                  </>
                )}
                <button onClick={handleAuth} disabled={authSubmitting || !authEmail || (authMode !== 'forgot' && !authPassword)}
                  ...
                  {authSubmitting ? 'Please wait...' : authMode === 'forgot' ? 'Send Reset Link' : authMode === 'login' ? 'Sign In' : 'Sign Up'}
                </button>
              </div>
              {authMode === 'login' && (
                <p style={{textAlign:'center',fontSize:12,marginTop:10,...t.textMuted}}>
                  <button onClick={() => { setAuthMode('forgot'); setAuthError(''); }}
                    style={{background:'none',border:'none',cursor:'pointer',color: t.accent,fontWeight:600,fontSize:12}}>
                    Forgot password?
                  </button>
                </p>
              )}
              <p style={{textAlign:'center',fontSize:13,marginTop: authMode === 'login' ? 8 : 16,...t.textMuted}}>
                {authMode === 'forgot' ? 'Remember your password? ' : authMode === 'login' ? "Don't have an account? " : 'Already have an account? '}
                <button onClick={() => { setAuthMode(authMode === 'signup' ? 'login' : authMode === 'forgot' ? 'login' : 'signup'); setAuthError(''); }}
        ...
        {!onboardingDone && step === 'home' && !showSettings && (() => {
          ...
          return (
            <div className="fade-up" style={{...fcol,gap:28}}>
              ...
              <div className={`${cardClass} border rounded-2xl shadow-sm`} style={{padding:'28px 24px',borderRadius:16,border:`1px solid ${borderLight}`,background: cardBg}}>
                ...
                {s.type === 'name' && (
                  <input type="text" placeholder="Your name..." value={userProfile.name} onChange={(e) => setUserProfile({...userProfile, name: e.target.value})}
                    className={`w-full px-4 py-3.5 rounded-xl border ${inputClass} text-lg`} style={{textAlign:'center',width:'100%',padding:'14px 16px',borderRadius:12,fontSize:18,fontFamily:'Manrope, sans-serif',background: darkMode ? '#1A1F2B' : '#fff',color: darkMode ? '#EFF1F5' : '#111827',border:`1px solid ${borderLight}`}} autoFocus />
                )}
                {s.type === 'dietary' && (
                  <div>
                    <div className="flex flex-wrap justify-center gap-2" style={{display:'flex',flexWrap:'wrap',justifyContent:'center',gap:8}}>
                      {['None', 'Vegetarian', 'Vegan', 'Gluten-Free', 'Dairy-Free', 'Keto', 'Paleo', 'Low-Carb', 'Pescatarian', 'Halal', 'Kosher'].map(opt => (
                        ...
                        }} className={`px-4 py-2.5 rounded-full text-sm font-medium transition ${userProfile.dietary.includes(opt) ? chipActiveClass : chipClass}`} style={{padding:'10px 16px',borderRadius:9999,fontSize:14,fontWeight:500,cursor:'pointer',border: userProfile.dietary.includes(opt) ? 'none' : `1px solid ${borderLight}`,fontFamily:'Manrope, sans-serif'}}>
                  ...
                  <div style={{...fcol,gap:10}}>
                    {[['beginner', '🔰', 'Beginner', 'Simple recipes, basic techniques'], ['intermediate', '👩‍🍳', 'Intermediate', 'Comfortable with most recipes'], ['advanced', '⭐', 'Advanced', 'Bring on the challenge']].map(([val, icon, label, desc]) => (
                      <button key={val} onClick={() => setUserProfile({...userProfile, skillLevel: val})}
                        ...
                        style={{...frow,gap:R.card,padding:'16px 18px',borderRadius:R.card,borderWidth:2,borderColor: userProfile.skillLevel === val ? (t.accent) : (borderCol),background: userProfile.skillLevel === val ? (accentBgSoft) : (cardBg),cursor:'pointer',textAlign:'left',fontFamily:'Manrope, sans-serif',width:'100%'}}>
                  ...
                  <div>
                    <div className="flex flex-wrap justify-center gap-2" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:8}}>
                      {['None', 'Dairy', 'Gluten', 'Nuts', 'Peanuts', 'Shellfish', 'Fish', 'Eggs', 'Soy'].map(a => (
                        ...
                        }} className={`px-4 py-2.5 rounded-full text-sm font-medium transition ${a === 'None' && allergens.length === 0 ? chipActiveClass : allergens.includes(a.toLowerCase()) ? 'bg-red-500 text-white shadow-md' : chipClass}`} style={{padding:'10px 16px',borderRadius:9999,fontSize:14,fontWeight:500,cursor:'pointer',border: (a === 'None' && allergens.length === 0) || allergens.includes(a.toLowerCase()) ? 'none' : `1px solid ${borderLight}`,fontFamily:'Manrope, sans-serif'}}>
              ...
              <div className="flex gap-3" style={{display:'flex',gap:12,width:'100%'}}>
                {onboardingStep > 0 && (
                  <button onClick={() => setOnboardingStep(onboardingStep - 1)} style={{flex:1,background: subtleBg2,borderRadius:16,padding:'16px 0',fontWeight:600,fontSize:15,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',...t.text}}>Back</button>
                )}
                <button onClick={() => {
                  if (onboardingStep < steps.length - 1) setOnboardingStep(onboardingStep + 1);
                  else completeOnboarding(userProfile);
                }} style={{flex:1,background: gradientBtn,color:'#fff',borderRadius:16,padding:'16px 0',fontWeight:700,fontSize:15,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',boxShadow: gradientShadow}}>
                  {onboardingStep < steps.length - 1 ? 'Continue' : "Let's Cook! 🎉"}
                </button>
              </div>
              {onboardingStep === 0 && (
                <button onClick={() => { setOnboardingDone(true); saveToStorage('onboarding-done', true); }} style={{padding:'8px 0',width:'100%',background:'none',border:'none',cursor:'pointer',fontSize:14,fontFamily:'Manrope, sans-serif',...t.textMuted}}>Skip for now</button>
              )}
            </div>
          );
        })()}
          ...
          <div style={{...fcol,gap:24}}>
            ...
            {!isPro && (
              <button onClick={handleUpgrade} disabled={upgrading} className="card-hover w-full fade-up-d1"
                style={{width:'100%',padding:'18px 20px',borderRadius:R.card,cursor:'pointer',fontFamily:'inherit',textAlign:'left',
        ...
        {step === 'capture' && (
          <div className="space-y-6 fade-up screen-enter">
            ...
            {/* Add photo buttons */}
            {images.length < MAX_PHOTOS && (
              <div className="grid gap-3" style={{display:'grid',gap:12,width:'100%'}}>
                <button onClick={() => cameraInputRef.current?.click()} className="card-hover" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:12,width:'100%',background: gradientBtn,color:'#fff',borderRadius:16,padding:'20px',border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',boxShadow: gradientShadow}}>
                  <Camera className="w-6 h-6" /><span style={{fontSize:18,fontWeight:700}}>Take Photo</span>
                </button>
                <button onClick={() => fileInputRef.current?.click()} className="card-hover" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:12,width:'100%',background: subtleBg,borderRadius:16,padding:'20px',border:`2px solid ${borderLight}`,cursor:'pointer',fontFamily:'Manrope, sans-serif'}}>
                  <Upload className="w-6 h-6" style={{color: t.accent}} /><span style={{fontSize:18,fontWeight:700,...t.text}}>Upload Photo</span>
                </button>
              </div>
            )}
            ...
            {images.length > 0 && (
              ...
              <button
                ...
                style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',background:'linear-gradient(to right, #10b981, #14b8a6)',color:'#fff',borderRadius:16,padding:'20px',fontWeight:700,fontSize:18,border:'none',cursor:'pointer',fontFamily:'Manrope, sans-serif',boxShadow:'0 8px 30px rgba(16,185,129,0.2)'}}
              >
            ...
            <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" onChange={handleImageCapture} style={{display:"none"}} />
        ...
        {step === 'pantry' && (
          <div style={{...fcol,gap:20}} className="screen-enter">
            ...
            <input ref={pantryCameraRef} type="file" accept="image/*" capture="environment" onChange={handlePantryScan} style={{display:"none"}} />
            <input ref={pantryFileRef} type="file" accept="image/*" onChange={handlePantryScan} style={{display:"none"}} />
            {/* Scan review */}
            {pantryScanReview && (
                    ...
                    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
                      ...
                      <div style={{display:'flex',gap:10}}>
                        <button onClick={() => activeTimer === currentStep ? pauseTimer() : setActiveTimer(currentStep)}
                          style={{padding:'10px 28px',borderRadius:12,fontSize:14,fontWeight:700,cursor:'pointer',
                ...
                <div style={{...fcol,gap:12}}>
                  <div style={{...frow,gap:12}}>
                    <div style={{width:40,height:40,borderRadius:10,background: isPro ? `linear-gradient(135deg, ${t.accent}, ${t.accentMuted})` : (accentBgAlt),...fc,fontSize:16,fontWeight:700,flexShrink:0,
                      ...
                      {isPro ? '⭐' : (userProfile.name?.[0]?.toUpperCase() || authUser.email?.[0]?.toUpperCase() || '?')}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{fontSize:14,fontWeight:600,...t.text,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{userProfile.name || authUser.email}</p>
                      <p style={{fontSize:12,...t.textMuted}}>{authUser.email} · {isPro ? 'Pro Member' : 'Free Plan'}</p>
                    </div>
                    <button onClick={() => { setEditName(userProfile.name || ''); setShowAccountEdit(!showAccountEdit); setShowChangePassword(false); setShowDeleteConfirm(false); }}
                      style={{padding:'6px 12px',borderRadius:8,fontSize:12,fontWeight:600,border:`1px solid ${borderCol}`,background:'transparent',cursor:'pointer',...t.textMuted}}>
                      Edit
                    </button>
                  </div>
                  ...
                  {/* Edit Profile */}
                  {showAccountEdit && (
                    <div style={{background: subtleBg,borderRadius:14,padding:16,...fcol,gap:10,border:`1px solid ${borderCol}`}}>
                      <p style={{fontSize:13,fontWeight:700,...t.text}}>Edit Profile</p>
                      <input type="text" placeholder="Display name" value={editName} onChange={e => setEditName(e.target.value)}
                        onKeyDown={e => e.key === 'Enter' && handleUpdateProfile()}
                        style={{width:'100%',padding:'12px 14px',borderRadius:10,fontSize:14,border:`1.5px solid ${borderCol}`,background: rootBg,color: textCol,outline:'none',boxSizing:'border-box'}} />
                      <div style={{...frow,gap:8}}>
                        <button onClick={handleUpdateProfile}
                          style={{flex:1,padding:'10px',borderRadius:10,fontSize:13,fontWeight:700,border:'none',cursor:'pointer',background: gradientBtn,color: gradientBtnCol}}>
                          Save
                        </button>
                        <button onClick={() => setShowAccountEdit(false)}
                          style={{padding:'10px 16px',borderRadius:10,fontSize:13,fontWeight:600,border:`1px solid ${borderCol}`,background:'transparent',cursor:'pointer',...t.textMuted}}>
                          Cancel
                        </button>
                      </div>
                    </div>
                  ...
                  {/* Action buttons row */}
                  <div style={{...frow,gap:8,flexWrap:'wrap'}}>
                    <button onClick={() => { setShowChangePassword(!showChangePassword); setShowAccountEdit(false); setShowDeleteConfirm(false); setNewPassword(''); setConfirmPassword(''); }}
                      style={{flex:1,minWidth:120,padding:'10px',borderRadius:10,fontSize:12,fontWeight:600,border:`1px solid ${borderCol}`,background:'transparent',cursor:'pointer',...t.textMuted}}>
                      🔑 Change Password
                    </button>
                  </div>
                  ...
                  {showChangePassword && (
                    <div style={{background: subtleBg,borderRadius:14,padding:16,...fcol,gap:10,border:`1px solid ${borderCol}`}}>
                      <p style={{fontSize:13,fontWeight:700,...t.text}}>Change Password</p>
                  ...
                  {/* Delete Account */}
                  <div style={{borderTop:`1px solid ${borderCol}`,paddingTop:12,marginTop:4}}>
                    {!showDeleteConfirm ? (
                      <button onClick={() => { setShowDeleteConfirm(true); setShowAccountEdit(false); setShowChangePassword(false); setDeleteConfirmText(''); }}
                        style={{background:'none',border:'none',cursor:'pointer',fontSize:12,fontWeight:600,color: darkMode ? '#EF4444' : '#DC2626',padding:0}}>
                        Delete my account
                      </button>
                    ) : (
                      <div style={{background: darkMode ? 'rgba(239,68,68,0.08)' : 'rgba(239,68,68,0.04)',borderRadius:12,padding:14,...fcol,gap:10,border:`1px solid ${darkMode ? 'rgba(239,68,68,0.2)' : 'rgba(239,68,68,0.15)'}`}}>
                        <p style={{fontSize:13,fontWeight:700,color: darkMode ? '#FCA5A5' : '#DC2626'}}>⚠️ This cannot be undone</p>
                        <p style={{fontSize:12,...t.textMuted}}>All your data, saved recipes, meal plans, and history will be permanently deleted.{isPro ? ' Your Pro subscription will also be cancelled.' : ''}</p>
                        <input type="text" placeholder='Type "DELETE" to confirm' value={deleteConfirmText} onChange={e => setDeleteConfirmText(e.target.value)}
                          style={{width:'100%',padding:'10px 12px',borderRadius:8,fontSize:13,border:`1.5px solid ${darkMode ? 'rgba(239,68,68,0.3)' : 'rgba(239,68,68,0.4)'}`,background: rootBg,color: textCol,outline:'none',boxSizing:'border-box'}} />
                        <div style={{...frow,gap:8}}>
                          <button onClick={handleDeleteAccount} disabled={deleteConfirmText !== 'DELETE'}
                            style={{flex:1,padding:'10px',borderRadius:10,fontSize:13,fontWeight:700,border:'none',cursor: deleteConfirmText === 'DELETE' ? 'pointer' : 'not-allowed',
                              background: deleteConfirmText === 'DELETE' ? '#EF4444' : (darkMode ? '#333' : '#DDD'),color: deleteConfirmText === 'DELETE' ? '#FFF' : (darkMode ? '#666' : '#999')}}>
                            Delete Account
                          </button>
          ...
          <div className="max-w-lg mx-auto p-4 pt-20 pb-24 space-y-5">
            ...
            {!mealScanResult ? (
              <div className="space-y-5 fade-up">
                ...
                <input ref={mealCameraInputRef} type="file" accept="image/*" capture="environment" onChange={handleMealScanCapture} style={{display:"none"}} />
                <input ref={mealScanInputRef} type="file" accept="image/*" onChange={handleMealScanCapture} style={{display:"none"}} />
              </div>
            ) : (
              <div className="space-y-5 fade-up">
